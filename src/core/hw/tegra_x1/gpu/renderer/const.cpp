...
u32 TextureDescriptor::GetLevelOffset(const u32 level) const {
    const auto& info = GetTextureFormatInfo(format);

    // u32 tile_width = 64 / info.bytes_per_block;
    // u32 tile_height = 8u << block_height_log2;
    // u32 tile_depth = 1u << block_depth_log2;

    u32 offset = 0;
    for (u32 i = 0; i < level; i++) {
        u32 level_width = AdjustSize(width, i, info.block_width);
        u32 level_height = AdjustSize(height, i, info.block_height);
        u32 level_depth = AdjustMipSize(depth, i);
        u32 level_width_bytes = level_width * info.bytes_per_block;

        u32 level_tile_w_shift =
            AdjustTileSize(block_width_log2, 64,
                           level_width_bytes); // TODO: block_width_log2 or 0?
        u32 level_tile_h_shift =
            AdjustTileSize(block_height_log2, 8, level_height);
        u32 level_tile_d_shift =
            AdjustTileSize(block_depth_log2, 1, level_depth);
        u32 level_tile_w_gobs = 1u << level_tile_w_shift;
        u32 level_tile_h_gobs = 1u << level_tile_h_shift;
        u32 level_tile_d = 1u << level_tile_d_shift;

        u32 level_width_gobs = ceil_divide(level_width_bytes, 64u);
        u32 level_height_gobs = ceil_divide(level_height, 8u);

        u32 level_width_tiles =
            (level_width_gobs + level_tile_w_gobs - 1) >> level_tile_w_shift;
        u32 level_height_tiles =
            (level_height_gobs + level_tile_h_gobs - 1) >> level_tile_h_shift;
        u32 level_depth_tiles =
            (level_depth + level_tile_d - 1) >> level_tile_d_shift;

        // TODO: sparse images

        offset += static_cast<u32>(level_width_tiles * level_height_tiles *
                                   level_depth_tiles)
                  << (9u + level_tile_w_shift + level_tile_h_shift +
                      level_tile_d_shift);
    }
...
constexpr u8 TextureDescriptor::AdjustTileSize(u8 shift, u8 unit_factor,
                                               u32 dimension) const {
    if (!shift)
        return 0;

    u32 x = u32(unit_factor) << (shift - 1);
    if (x >= dimension) {
        while (--shift) {
            x >>= 1;
            if (x < dimension)
