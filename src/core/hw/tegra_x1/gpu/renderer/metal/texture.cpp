...
Texture::Texture(const TextureDescriptor& descriptor)
    ...
    if (pixel_format_info.component_indices == uchar4{0, 1, 2, 3}) {
        ...
        owns_base = true;
        texture = CreateViewImpl(
            TextureViewDescriptor(descriptor.format, SwizzleChannels(),
...
TextureBase* Texture::CreateView(const TextureViewDescriptor& descriptor) {
    ...
    return new Texture(desc, CreateViewImpl(descriptor));
}
...
void Texture::CopyFrom(ICommandBuffer* command_buffer, const BufferBase* src,
    ...
    for (u32 i = 0; i < layers.GetSize(); i++) {
        const auto crnt_dst_level = levels.GetBegin() + i;
        const auto crnt_dst_layer = layers.GetBegin() + i;
        encoder->copyFromBuffer(
            mtl_src,
            crnt_dst_layer * descriptor.layer_stride +
                descriptor.GetLevelOffset(crnt_dst_level),
            src_stride, align(descriptor.height, 16u) * src_stride,
            MTL::Size(size.x(), size.y(), size.z()), texture, crnt_dst_layer,
            crnt_dst_level,
...
MTL::Texture* Texture::CreateViewImpl(const TextureViewDescriptor& descriptor) {
    const auto& pixel_format_info = to_mtl_pixel_format_info(descriptor.format);
    ...
    return base_texture->newTextureView(
        pixel_format_info.pixel_format, ToMtlTextureType(this->descriptor.type),
        NS::Range(descriptor.levels.GetBegin(),
